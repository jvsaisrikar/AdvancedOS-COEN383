/*  To compile:
 *   gcc project3.c queue.c -lpthread -o p3
 *
 *   To run:
 *   ./p3 <N>
...
void *sell(void *seller)
{
    ...
    while (current_time_slice < sim_duration)
    {
        ...
        // Sell
        if (current_time_slice == sim_duration)
            break;
        ...
        if (c == NULL && sellerQueue->size > 0)
        {
            ...
            printf("00:%02d     %c%d        Serving Customer Num %c%d%02d       %8d\n",
        ...
        if (c != NULL)
        {
            if (randomWaitTime == 0)
            {
                ...
                if (seatIdx == -1)
                {
                    printf("00:%02d     %c%d        Customer No %c%d%02d has been told concert is sold out.\n",
                ...
                else
                {
                    ...
                    printf("00:%02d     %c%d        Customer Num %c%d%02d assigned seat %d,     %d %15d\n",
...
int main(int argc, char **argv)
{
    ...
    for (int r = 0; r < CONCERT_ROW; r++)
    {
        for (int c = 0; c < CONCERT_COL; c++)
        {
            ...
            if (seats_matrix[r][c][0] == 'H')
                h_customers++;
            if (seats_matrix[r][c][0] == 'M')
                m_customers++;
            if (seats_matrix[r][c][0] == 'L')
                l_customers++;
        }
        printf("\n");
    }
